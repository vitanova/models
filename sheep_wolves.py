...
chose_default = input("chosing default settings with \n" + 
    ...
    "number of peroids simulating = 60 \n"
    "Y / N ? ")
if chose_default == 'Y':
    length = 20
    sheep = 50
    ...
    T = int(input("number of peroids simulating: "))
total = sheep + wolves
...
color = 'tab10'
fig = plt.figure()
img = plt.imshow(prairie, interpolation='nearest', animated=True)
img.set_cmap(color)
plt.colorbar()
plt.axis('off')
...
for t in range(T):
    before = current
    img = plt.imshow(before, interpolation='nearest', animated=True)
    img.set_cmap(color)
    txt = plt.text(10, 10, 'ROUND '+str(t), fontsize=20)
    ims.append([img, txt])
