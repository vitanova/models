...
@jit(nopython=True)
def u(c, l):
    return (c-phi_1/(1+phi_2)*l**(1+phi_2))**(1-sigma)/(1-sigma)
        ...
        for iz in range(N_z):
            for ik in range(grid):
                ...
                for ikprime in range(grid):
                    l = ((1-alpha)*np.exp(ZZ[iz])*A*K[ik]**(alpha)*1/phi_1)**(1/(alpha+phi_2))
...
# labor as function of k and z
def l(k, z):
    return (A*np.exp(z)*k**alpha*(1-alpha)/phi_1)**(1/(alpha+phi_2))
# labor for all grid of z and k
LL = np.empty((N_z, grid))
for iz in range(N_z):
    for ik in range(grid):
        LL[iz, ik] = l(K[ik], ZZ[iz])
...
for i in range(3):
    for iz in range(5):
        if i==0:
            ...
            axes[i].plot(K, LL[4-iz], label='z=z('+str(4-iz)+')')
